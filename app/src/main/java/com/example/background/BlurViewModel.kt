...
class BlurViewModel(application: Application) : ViewModel() {
    ...
    internal fun applyBlur(blurLevel: Int) {
        ...
        var continuation = workManager.beginUniqueWork(IMAGE_MANIPULATION_WORK_NAME, ExistingWorkPolicy.REPLACE ,cleanupWorker)
        ...
        for (i in 0 until blurLevel){
            ...
            if(i == 0)
                blurRequest.setInputData(data)

            continuation = continuation.then(blurRequest.build())

        }

        val saveImageToFileWorker = OneTimeWorkRequestBuilder<SaveImageToFileWorker>()
